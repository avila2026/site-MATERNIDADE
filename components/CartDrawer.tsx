...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose, items, onRemoveItem, onCheckout }) => {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div 
        className={`fixed inset-y-0 right-0 w-full md:w-[450px] bg-[#FFF1F2] z-[70] shadow-2xl transform transition-transform duration-500 ease-in-out border-l border-[#FECDD3] flex flex-col ${
          isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[#FECDD3]">
          <h2 className="text-xl font-serif text-[#4C0519]">Seu Carrinho ({items.length})</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="w-12 h-12 text-[#FDA4AF]">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007zM8.625 10.5a.375.375 0 11-.75 0 .375.375 0 01.75 0zm7.5 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
              </svg>
              <p className="font-light text-[#881337]">Seu carrinho está vazio.</p>
            </div>
          ) : (
            items.map((item, idx) => (
              <div key={`${item.id}-${idx}`} className="flex gap-4 animate-fade-in-up">
                <div className="w-20 h-24 bg-[#FFE4E6] flex-shrink-0">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <button 
                    ...
                    className="text-xs text-[#FDA4AF] hover:text-[#4C0519] self-start underline underline-offset-4 transition-colors"
                  >
                    Remover
                  </button>
                </div>
        ...
        <div className="p-6 border-t border-[#FECDD3] bg-[#FFE4E6]/30">
          ...
          <button 
            ...
            className="w-full py-4 bg-[#4C0519] text-[#FFF1F2] uppercase tracking-widest text-sm font-medium hover:bg-[#881337] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Finalizar Compra
          </button>
        </div>
