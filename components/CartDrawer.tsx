...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose, items, onRemoveItem, onCheckout }) => {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div 
        className={`fixed inset-y-0 right-0 w-full md:w-[450px] bg-[#E0F2F1] z-[70] shadow-2xl transform transition-transform duration-500 ease-in-out border-l border-[#B2DFDB] flex flex-col ${
          isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[#D6D1C7]">
          <h2 className="text-xl font-serif text-[#2C2A26]">Seu Carrinho ({items.length})</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              ...
              <p className="font-light text-[#5D5A53]">Seu carrinho está vazio.</p>
            </div>
          ) : (
            items.map((item, idx) => (
        ...
        <div className="p-6 border-t border-[#D6D1C7] bg-[#EBE7DE]/30">
          ...
          <p className="text-xs text-[#A8A29E] mb-6 text-center">Frete e impostos calculados no checkout.</p>
          <button 
            onClick={onCheckout}
